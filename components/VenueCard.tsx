...
export default function VenueCard({ venue, rank, isActive, onHover, onLeave }: VenueCardProps) {
  ...
  return (
    ...
    >
      <article className={`group relative bg-white border rounded-2xl p-5 hover:shadow-lg transition-all duration-200 cursor-pointer h-full flex flex-col ${
        isActive ? "border-zinc-900 shadow-md ring-1 ring-zinc-900" : "border-zinc-100 hover:border-zinc-200"
      }`}>
        {/* Rank badge */}
        <span className="absolute top-4 right-4 w-6 h-6 flex items-center justify-center rounded-full bg-zinc-50 border border-zinc-200 text-xs font-semibold text-zinc-400">
          {rank}
        </span>
        ...
        <div className="pr-8 mb-2">
          ...
          <p className="text-xs text-zinc-400 mt-0.5 font-medium tracking-wide uppercase">
            {venue.suburb}
          </p>
        </div>
        ...
        <div className="space-y-1.5 text-xs text-zinc-500">
          ...
          {genreDisplay && (
            <div className="flex items-center gap-2">
              <svg className="w-3.5 h-3.5 text-zinc-300 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
          ...
          {venue.entryFee && (
            <div className="flex items-center gap-2">
              <svg className="w-3.5 h-3.5 text-zinc-300 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M5 5a3 3 0 0 1 5.268-1.954A3 3 0 1 1 9 8.817V9h2v1H9v1h2v1H9v3H8v-3H6v-1h2v-1H6v-1h2v-.183A3 3 0 0 1 5 5Zm3-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2Zm4 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z" clipRule="evenodd" />
              </svg>
